	...
	public ResponseEntity<?> userLogin(@Valid @RequestBody LoginRequest request) {
		try {
			...
			if (authentication.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_USER"))) {
				...
				return ResponseEntity.ok(new JwtAuthenticationResponse(jwtToken, expiration, newRefreshToken.getRefreshToken()));
			}
		} catch (BadCredentialsException e) {
			throw new PasswordMismatchException("Incorrect password!");
		}
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ApiResponse(false, "Username or password incorrect."));
	}
}
